document.getElementById('donate-btn3').addEventListener('click', () => {
    //get value from input field
    const donationAmount = getInputValueById('donationAmount3');
    //get value from element
    const totalDonationValue = getElementValueById('totalDonation3');
    //get total remaining money
    const remainingMoney = getElementValueById('remainingMoney');
    //get card Title
    const cardTitle = getInnerTextById('card-title3');
    //get time
    const now = new Date();
    ...
    if (donationAmount > 0) {
        if (donationAmount > remainingMoney) {
            ...
            document.getElementById('totalDonation3').innerText = totalDonationValue + donationAmount;
